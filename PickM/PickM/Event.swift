...
class eventLoad {
    let webKey = "30EB9854800661241D12932808F0C408"
    var steamId = "76561198034432280"
    var userKey = "9KMG-8TUH6-ZWRE"
    var players: [Player] = []
    ...
    func parseEventLayout(_ json: JSON) {
        ...
        for result in json["result"]["sections"].arrayValue {
            ...
            eventDat.days[n].name = sectionName
            eventDat.days[n].id = sectionId
            var x = 0
            
            for group in json["result"]["sections"][n]["groups"].arrayValue {
                ...
                eventDat.days[n].matches[x].id = groupId
                eventDat.days[n].matches[x].name = groupName
                eventDat.days[n].matches[x].pointsPerPick = pointsPerPick
                eventDat.days[n].matches[x].picksAllowed = picksAllowed
                var y = 0
                
                
                for team in json["result"]["sections"][n]["groups"][x]["teams"].arrayValue {
                    let teamId = team["pickid"].intValue
                    eventDat.days[n].matches[x].teams[y].id = teamId
                    y += 1
                }
                
                for pick in json["result"]["sections"][n]["groups"][x]["picks"].arrayValue {
                    let pickIndex = pick["index"].intValue
                    let pickId = pick["pickids"][0].intValue
                    eventDat.days[n].matches[x].pickIndex = pickIndex
                    eventDat.days[n].matches[x].pickIds = pickId
        ...
        for day in eventDat.days {
            ...
            for match in day.matches {
                ...
                for team in match.teams {
                    switch team.id {
                    case 1?:
                        eventDat.days[x].matches[y].teams[z].logo = UIImage(named: "nip")
                        eventDat.days[x].matches[y].teams[z].name = "Ninjas In Pyjamas"
                        eventDat.days[x].matches[y].teams[z].players = teamInfo(teamId: team.id!)
                        break
                    case 6?:
                        eventDat.days[x].matches[y].teams[z].logo = UIImage(named: "fntc")
                        eventDat.days[x].matches[y].teams[z].name = "Fnatic"
                        eventDat.days[x].matches[y].teams[z].players = teamInfo(teamId: team.id!)
                        break
                    case 12?:
                        eventDat.days[x].matches[y].teams[z].logo = UIImage(named: "navi")
                        eventDat.days[x].matches[y].teams[z].name = "Natus Vincere"
                        eventDat.days[x].matches[y].teams[z].players = teamInfo(teamId: team.id!)
                        break
                    case 14?:
                        eventDat.days[x].matches[y].teams[z].logo = UIImage(named: "sk")
                        eventDat.days[x].matches[y].teams[z].name = "SK Gaming"
                        eventDat.days[x].matches[y].teams[z].players = teamInfo(teamId: team.id!)
                        break
                    case 24?:
                        eventDat.days[x].matches[y].teams[z].logo = UIImage(named: "dig")
                        eventDat.days[x].matches[y].teams[z].name = "Team Dignitas"
                        eventDat.days[x].matches[y].teams[z].players = teamInfo(teamId: team.id!)
                        break
                    case 29?:
                        eventDat.days[x].matches[y].teams[z].logo = UIImage(named: "mss")
                        eventDat.days[x].matches[y].teams[z].name = "mousesports"
                        eventDat.days[x].matches[y].teams[z].players = teamInfo(teamId: team.id!)
                        break
                    case 31?:
                        eventDat.days[x].matches[y].teams[z].logo = UIImage(named: "vp")
                        eventDat.days[x].matches[y].teams[z].name = "Virtus.Pro"
                        eventDat.days[x].matches[y].teams[z].players = teamInfo(teamId: team.id!)
                        break
                    case 43?:
                        eventDat.days[x].matches[y].teams[z].logo = UIImage(named: "flip")
                        eventDat.days[x].matches[y].teams[z].name = "Flipsid3 Tactics"
                        eventDat.days[x].matches[y].teams[z].players = teamInfo(teamId: team.id!)
                        break
                    case 46?:
                        eventDat.days[x].matches[y].teams[z].logo = UIImage(named: "nv")
                        eventDat.days[x].matches[y].teams[z].name = "Team EnVyUs"
                        eventDat.days[x].matches[y].teams[z].players = teamInfo(teamId: team.id!)
                        break
                    case 48?:
                        eventDat.days[x].matches[y].teams[z].logo = UIImage(named: "liq")
                        eventDat.days[x].matches[y].teams[z].name = "Team Liquid"
                        eventDat.days[x].matches[y].teams[z].players = teamInfo(teamId: team.id!)
                        break
                    case 49?:
                        eventDat.days[x].matches[y].teams[z].logo = UIImage(named: "clg")
                        eventDat.days[x].matches[y].teams[z].name = "Counter Logic Gaming"
                        eventDat.days[x].matches[y].teams[z].players = teamInfo(teamId: team.id!)
                        break
                    case 59?:
                        eventDat.days[x].matches[y].teams[z].logo = UIImage(named: "g2")
                        eventDat.days[x].matches[y].teams[z].name = "G2 Esports"
                        eventDat.days[x].matches[y].teams[z].players = teamInfo(teamId: team.id!)
                        break
                    case 60?:
                        eventDat.days[x].matches[y].teams[z].logo = UIImage(named: "astr")
                        eventDat.days[x].matches[y].teams[z].name = "Astralis"
                        eventDat.days[x].matches[y].teams[z].players = teamInfo(teamId: team.id!)
                        break
                    case 61?:
                        eventDat.days[x].matches[y].teams[z].logo = UIImage(named: "faze")
                        eventDat.days[x].matches[y].teams[z].name = "FaZe Clan"
                        eventDat.days[x].matches[y].teams[z].players = teamInfo(teamId: team.id!)
                        break
                    case 63?:
                        eventDat.days[x].matches[y].teams[z].logo = UIImage(named: "gamb")
                        eventDat.days[x].matches[y].teams[z].name = "Gambit Gaming"
                        eventDat.days[x].matches[y].teams[z].players = teamInfo(teamId: team.id!)
                        break
                    case 66?:
                        eventDat.days[x].matches[y].teams[z].logo = UIImage(named: "optc")
                        eventDat.days[x].matches[y].teams[z].name = "OpTic Gaming"
                        eventDat.days[x].matches[y].teams[z].players = teamInfo(teamId: team.id!)
                        break
                    default:
                        break
...
class Event {
    var eventId = 10
    var name: String?
    var days = [Day(), Day(), Day(), Day(), Day(), Day(), Day()]
}

class Day {
    var id: Int?
    var name: String?
    var matches = [Match(), Match(), Match(), Match(), Match(), Match(), Match(), Match()]
}
